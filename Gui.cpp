...
void Gui::InitButton() {
	string buttons[NUM_BUTTON] = { "play.png", "music.png", "quit.png" };
	string hover_buttons[NUM_BUTTON] = { "pressplay.png", "pressmusic.png", "pressquit.png" };
...
void Gui::RenderButton() {
	...
	for (int i = 0; i < NUM_BUTTON; i++) {
		...
		mat4 model;
		model = translate(model, vec3((sw - button_width[i]) / 2, (i + 1) * 130, 0.0f));
